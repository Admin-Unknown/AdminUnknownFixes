...
| 0-auf-extend-guard | Has to load before every other mod, which AdminUnknownFixes cannot: requiring pycoalprocessing holds it behind everything that sorts earlier | [0-auf-extend-guard_0.0.3.zip](https://github.com/jakegodding/AdminUnknownFixes/raw/main/stubs/0-auf-extend-guard_0.0.3.zip) |
...
From 0.0.3 it does a second job for Angel’s. Early in the data-updates stage Angel’s walks every recipe in the game and reads each ingredient and each result as a table with a name in it; anything written the way Factorio 1.1 allowed stops the load with `table index is nil` in `angelsrefining/prototypes/override-functions.lua`. AdminUnknownFixes carries the pass that puts those entries into the form 2.0 asks for, but it loads after Angel’s, so the guard calls it at the top of that stage, which is the last moment before Angel’s looks. Without the guard, that pass still runs, but only in time for the stages after the one that breaks.
