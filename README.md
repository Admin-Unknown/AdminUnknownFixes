...
From 0.0.3 it also runs AdminUnknownFixes’ recipe shape pass at the top of the data-updates stage. That is belt and braces: since 2.1.27 the main mod puts that pass in front of each of Angel’s override passes itself, wherever and whenever they are called, so the guard is not needed for the `table index is nil` failure in `angelsrefining/prototypes/override-functions.lua`. Only the bobpower failure above needs it.
